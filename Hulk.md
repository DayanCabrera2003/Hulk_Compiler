...
Although HULK's syntax contains no explicit notion of memory allocation or deallocation (there is no `malloc`, `free`, or `delete` anywhere in the language), every non-trivial HULK program allocates memory dynamically. Every `new` expression, every string concatenation, every vector literal or generator pattern, and every lambda expression produces a value that must live somewhere in memory. Since HULK gives no tools to the programmer to reclaim that memory, the language **requires** that the runtime provide an automatic garbage collector (GC) as part of any conforming implementation.

This section specifies the garbage collection model that a HULK compiler is expected to provide. It does not prescribe a single implementation, but rather defines the observable semantics of memory management and then describes a simple reference algorithm — a tracing mark-and-sweep collector with a shadow stack — that is known to satisfy those semantics. A compiler writer is free to implement a more sophisticated collector (generational, incremental, concurrent, etc.) as long as the observable behavior of correct programs is preserved.

Crucially, **the GC does not modify HULK's syntax in any way**. Everything described here is a property of the runtime and the code generator. A HULK program that is valid without a GC is valid with one, and vice versa.

## Value semantics and reference semantics

HULK values fall into two categories for the purposes of memory management:

- **Value types** are values that fit in a machine word (or a small fixed-size representation) and are copied whenever they are assigned, passed as an argument, or returned. The builtin types `Number` and `Boolean` are value types. They are never managed by the GC.
- **Reference types** are values that live in a dynamically allocated region of memory (the *heap*). Variables, attributes, parameters, and return values of reference type actually store a *pointer* to the heap location. All of the following are reference types and therefore managed by the GC:
  - Instances of user-defined types created with `new`.
  - Strings, including all string literals and the result of every `@` concatenation.
  - Vectors, including both explicit (`[a, b, c]`) and implicit (`[e | x in it]`) forms.
  - Closures produced by lambda expressions and by the implicit functor-wrapping of functions.

The distinction is entirely an implementation concern and is **not observable from HULK source code**. Two variables of reference type can point to the same object, but since user types have private attributes and no built-in aliasing operator, programs cannot directly test identity. Value types and reference types are interchangeable at the language level: both appear as ordinary expressions.

## What the GC guarantees

A conforming HULK runtime guarantees the following:

1. **No manual deallocation.** Programs never need to free memory, and have no syntax to do so.
2. **No use-after-free.** Any object that is still *reachable* (see below) is guaranteed to remain valid. Dereferencing a reference-typed variable is always safe as long as the variable holds a value obtained from a well-defined HULK expression.
3. **Eventual reclamation.** Any object that becomes unreachable will, at some later point, have its memory reclaimed by the runtime. The exact moment is unspecified.
4. **Finalization is not guaranteed.** HULK has no destructors or finalizers. The GC may reclaim an object's memory without running any HULK code on it.

Note that "eventual reclamation" is deliberately weak: a valid implementation is one that never collects (leaking memory), as long as it doesn't crash. A *useful* implementation is expected to reclaim memory promptly enough that long-running programs don't exhaust the heap.

## Reachability and roots

The correctness of the GC rests on a single concept: **reachability**. An object `o` is *reachable* if and only if there is a chain of pointers starting from a *root* and ending at `o`. The *roots* of a HULK program at any given moment are the union of the following:

- The values currently held by all **local variables** of all active `let` expressions (including nested ones) in all currently executing functions, methods, and macro expansions.
- The values currently held by all **parameters** of the active function, method, or macro invocations.
- The value of the **`self` symbol** inside any currently executing method.
- The values of all **global variables and constants**, including the entries in the string-literal pool and the references to builtin singletons (e.g. `PI` and `E` are value-typed and irrelevant here, but a future builtin of reference type would count).
- Any object **temporarily in use** by a partially evaluated expression — for example, the value of the left operand of `@` while the right operand is being evaluated, or the instance on which a method is about to be dispatched.

An object that is not reachable cannot influence the future behavior of the program in any way, and is therefore a valid candidate for reclamation.

For a compiler to correctly implement the GC, it must be able to enumerate the roots at every point where the GC might run. Section [The shadow stack](#the-shadow-stack) below describes a simple technique for doing this.

## Object layout

Every heap-allocated object in HULK carries, in addition to its payload, a small **object header** used by the runtime. The exact bits and sizes are implementation-defined, but the header must convey at least the following information:

```
+-----------------------------+
|  type tag / vtable pointer  |   identifies the type, methods, and attribute layout
+-----------------------------+
|  size in bytes              |   total size of this object (header + payload)
+-----------------------------+
|  mark bit(s)                |   used by the collector during a GC cycle
+-----------------------------+
|  payload                    |
|   ...attributes,            |
|      string bytes,          |
...
The **type tag** (or a vtable pointer, if method dispatch is done via vtables) allows the GC to know, given an arbitrary heap pointer, which fields inside the payload are themselves pointers to other heap objects. This is what enables the collector to trace references recursively. For each type, the compiler must emit a static *pointer map* — a description, typically a bitmap or an offset list, of which attribute slots in that type's instances hold references as opposed to value-typed data.

For the four kinds of managed objects, the pointer maps are:

- **User type instances.** Determined by the attribute declarations. An attribute typed `Number` or `Boolean` is a non-pointer slot; any other attribute type is a pointer slot.
- **Strings.** Contain no pointer slots in the payload (they are raw byte arrays plus a length).
- **Vectors.** If the element type is a value type, the payload has no pointer slots. If the element type is a reference type, every element is a pointer slot. This information is known statically because vectors in HULK are homogeneous (`T*` or `T[]`).
- **Closures.** Contain a code pointer (not traced as a heap reference) plus a captured environment; every captured variable of reference type is a pointer slot.

## The reference algorithm: mark-and-sweep

HULK's reference collector is a classical stop-the-world **tracing mark-and-sweep** collector. It operates in two phases:

**Phase 1: Mark.** Starting from the roots, traverse the object graph and set the mark bit on every object that is reached.
...
mark(obj):
    ...
    if obj.header.mark_bit is set: return
    obj.header.mark_bit := 1
    for each pointer slot p in obj (according to obj's pointer map):
        mark(*p)
```

**Phase 2: Sweep.** Walk the list of all allocated objects. For each one:

- If its mark bit is set, clear the bit (ready for the next cycle) and keep the object.
- If its mark bit is clear, the object is unreachable: return its memory to the allocator's free list.

```
sweep_phase():
    for each obj in allocated_list:
        if obj.header.mark_bit is set:
            obj.header.mark_bit := 0
        else:
            free(obj)
            remove obj from allocated_list
```

To make these phases possible, the runtime maintains a global **allocated list** (e.g., an intrusive linked list threaded through every heap object, or an auxiliary data structure) that tracks every live allocation.

## The shadow stack

Identifying the roots is the most delicate part of implementing a tracing GC. The reference technique prescribed by HULK is the **shadow stack**, which is simple to implement on essentially any target platform (native code, C, a virtual machine) without requiring precise stack maps or debug metadata.

A shadow stack is an auxiliary data structure, maintained by the runtime, that lists all reference-typed values currently live in the program. The compiler emits code to push a slot onto the shadow stack at the beginning of every scope that introduces a reference-typed variable, and to pop it when the scope ends.

Concretely, the compiler's code generator follows these rules when lowering HULK to the target language:

1. At the **entry** of every function, method, and macro expansion, allocate a shadow-stack frame containing one slot per reference-typed parameter and per reference-typed local variable introduced by any `let` in the body.
2. Every **assignment** to a reference-typed variable (including the initial binding of a `let`, and `:=` reassignment) writes both to the real variable location and to the corresponding shadow-stack slot.
3. **Short-lived temporaries** produced during expression evaluation (for example, the intermediate strings in `a @ b @ c`) must also be kept alive while they might be used. The simplest rule is: before every call that could trigger a GC, ensure all live temporaries are reachable from the shadow stack.
4. At the **exit** of every function, method, or macro expansion, pop the shadow-stack frame.
5. The `self` symbol is treated like any other reference-typed parameter.
6. Global reference-typed values live in a separate, statically allocated root region, not in the shadow stack.

Given this discipline, the GC's root enumeration is trivial: walk the shadow stack plus the global root region.

The cost of the shadow stack is a small constant overhead per function call and per assignment. This overhead is accepted in exchange for implementation simplicity. A more advanced implementation may replace the shadow stack with precise stack maps generated alongside the machine code; this is an optimization that does not affect the language's semantics.

## When does the GC run?

A HULK program triggers garbage collection under the following conditions:

- **Heap threshold.** The runtime tracks the total bytes currently allocated. When a new allocation would cause this total to exceed a threshold, the GC runs before the allocation is attempted. After a collection, the threshold is adjusted — typically to some multiple (e.g. 2×) of the currently live size — so that the GC is called less frequently as the live set grows.
- **Allocation failure.** If even after a collection there is insufficient memory to satisfy an allocation, the runtime aborts with an out-of-memory error.
- **Explicit request.** The runtime may, but is not required to, expose a builtin function `gc()` of type `() -> Object` (returning a dummy value) that forces an immediate collection. This is useful for debugging and benchmarking. Because this would be an ordinary builtin function invocation, its presence does not affect HULK's syntax.

Between these triggers, allocation is a fast-path operation: typically just bumping a pointer or popping a free-list entry.

## Interaction with specific language features

**`new` expressions.** Every `new T(args)` desugars, at the runtime level, to: (a) evaluate the arguments; (b) request an allocation of the appropriate size from the allocator; (c) fill in the header; (d) run attribute initializers in the freshly allocated payload. The allocator is the sole entry point through which the heap threshold is checked.

**String literals and concatenation.** String literals are interned at program startup into an immortal region (they are always roots). Each `@` operation allocates a fresh heap string whose size is the sum of its operands. Because strings are immutable, the runtime is free to reuse storage through more advanced schemes, but the simple implementation is "always allocate fresh."

**Vectors.** Explicit vectors `[e1, ..., en]` allocate a block sized for `n` elements, plus the header. Implicit vectors `[e | x in it]` have statically unknown size and must either: (a) be built by iterating the source into a dynamically growing buffer (which itself requires allocations); or (b) be consumed lazily as an iterable. The reference implementation uses strategy (a). Growing buffers produce short-lived garbage (the old backing arrays), which the GC will reclaim on the next cycle.

**Closures.** A lambda expression allocates a closure object whose environment contains copies of, or references to, every free variable captured from the enclosing scope. Captured variables of reference type are pointer slots in the environment; captured variables of value type are not. Implicit functor wrappers (when a function is passed where a functor is expected) allocate no environment and may be implemented as a statically allocated singleton.

**Macros.** Macros are expanded at compile time and produce no special runtime artifacts. Any allocations performed inside a macro's expansion obey the rules above for the constructs the expansion uses.

**`is` and `as`.** Dynamic type testing and downcasting do not allocate and do not interfere with the GC. They read the type tag from the object header.

## A note on cycles

Mark-and-sweep collectors reclaim cyclic garbage correctly with no special effort, because reachability is determined from the roots, not from reference counts. A cycle of mutually referring objects that is not reachable from any root will simply fail to be marked in the mark phase, and will be swept like any other garbage. This is one of the main reasons mark-and-sweep is specified as the reference algorithm for HULK: it is strictly more expressive than naive reference counting, at a comparable level of implementation difficulty.

## Summary for compiler implementors

To implement the HULK GC, a compiler must, at minimum:

- Distinguish value types (`Number`, `Boolean`) from reference types at every point where a value is produced, stored, or passed.
- Emit a type descriptor for each type, protocol-backing type, vector element type, and closure type, containing at least a size and a pointer map.
- Emit code that allocates every managed object through a single runtime `alloc(size, type_tag)` entry point.
- Emit shadow-stack push/pop code at every function, method, and macro-expansion boundary, and keep the shadow stack synchronized with live reference-typed variables.
- Link against a runtime library that provides `alloc`, the mark-and-sweep implementation, the allocated list, the global root region, and (optionally) the `gc()` builtin.

With these pieces in place, every HULK program — regardless of which subset of the language it uses — will run with fully automatic memory management, and no change to the HULK source language is required.
