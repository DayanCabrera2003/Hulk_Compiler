...
impl Resolver {
    ...
    fn report_ambiguous_function_inference(&mut self, function: &FunctionDecl) {
    ...
    fn validate_expr_against_annotation(&mut self, expr: &Expr, annotation: Option<&TypeAnn>) {
    ...
    fn validate_method_call(&mut self, receiver: &Expr, call_expr: &Expr) {
        let ExprKind::MethodCall { method, .. } = &call_expr.kind else {
            return;
        };

        let receiver_type = self.resolve_concrete_type_symbol(receiver);
        ...
        if !self.type_has_method(type_id, method) {
            self.bag.push(
                Diagnostic::error(format!("metodo no existe: {method}"))
                    .with_label(call_expr.span.clone(), "el receptor no define ese método"),
            );
        }
    }

    fn validate_call_argument_protocol_conformance(
        &mut self,
    ...
    fn resolve_concrete_type_symbol(&self, expr: &Expr) -> Option<SymbolId> {
        match &expr.kind {
            ExprKind::New {
    ...
    fn is_protocol_symbol(&self, symbol_id: SymbolId) -> bool {
        self.table
            .get(symbol_id)
            .is_some_and(|symbol| matches!(symbol.kind, SymbolKind::Protocol))
    }

    fn type_has_method(&self, type_id: SymbolId, method_name: &str) -> bool {
        if self
            .type_methods
            .get(&type_id)
    ...
    fn type_conforms_protocol(&self, type_id: SymbolId, protocol_id: SymbolId) -> bool {
    ...
    fn collect_protocol_methods(&self, protocol_id: SymbolId) -> HashSet<String> {
