...
fn fib_generates_recursive_call() {
    ...
    let fib_fn = prog.functions.iter().find(|f| f.name == "fib")
        .expect("fib function not found");
    assert!(
        has_call_to(&fib_fn.body, "fib"),
        "fib should have a recursive Call to Global(\"fib\"): {:?}", fib_fn.body
    );
}
...
fn new_expr_generates_new_instr() {
    ...
    assert!(
        ...
        "main should contain New {{ type_name: \"Point\" }}: {:?}", prog.main.body
    );
}
...
fn field_access_generates_get_field() {
    ...
    assert!(
        ...
        "main should contain GetField {{ field: \"v\" }}: {:?}", prog.main.body
    );
}
...
fn method_call_generates_method_call_instr() {
    ...
    assert!(
        ...
        "main should contain MethodCall with method \"get\": {:?}", prog.main.body
    );
}
...
fn arithmetic_produces_binop_instrs() {
    ...
    let has_binop = prog.main.body.iter().any(|i| matches!(i, Instr::BinOp { .. }));
...
fn type_decl_generates_init_method() {
    ...
    let foo_type = prog.types.iter().find(|t| t.name == "Foo")
