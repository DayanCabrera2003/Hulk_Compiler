...
fn count_labels(instrs: &[Instr]) -> usize {
    instrs.iter().filter(|i| matches!(i, Instr::Label(_))).count()
}

fn count_jumpif(instrs: &[Instr]) -> usize {
    instrs.iter().filter(|i| matches!(i, Instr::JumpIf { .. })).count()
}

fn has_loop_label(instrs: &[Instr]) -> bool {
    instrs.iter().any(|i| {
        if let Instr::Label(name) = i { name.starts_with("loop_") } else { false }
    })
}
...
fn if_generates_jumpif_and_labels() {
    ...
    assert!(count_jumpif(&prog.main.body) >= 1, "if needs at least one JumpIf");
    assert!(count_labels(&prog.main.body) >= 2, "if needs at least two Labels (then + end)");
}
...
fn elif_branch_generates_extra_jumpif_and_labels() {
    ...
    assert!(
        ...
        count_jumpif(&prog.main.body), prog.main.body
    );
...
fn while_generates_loop_label_and_jumpif() {
    let prog = support::build_banner(
        "while_test",
        "let x = 0 in while (x < 5) { x := x + 1; };",
    );
    assert!(has_loop_label(&prog.main.body), "while needs a loop_ label");
    assert!(count_jumpif(&prog.main.body) >= 1, "while needs a JumpIf exit");
}
...
fn let_shadowing_resolves_inner_binding() {
    ...
    let has_copy = prog.main.body.iter().any(|i| matches!(i, Instr::Copy { .. }));
